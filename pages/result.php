...
<body>
    ...
    <div class="container">
        <?php
        $stmt = $conn->prepare("SELECT * FROM placemeta WHERE name LIKE '%$search%'");
        $stmt->execute();
        ...
        if ($stmt->rowCount() > 0) {
            while ($row = $stmt->fetch()) {
                ...
                echo "<button id='book-now' class='button-price'> Check available price</button >";
                echo "</div>";
                echo "</div >";
                echo "</div >";
            }
        } else {
            echo "<h1 class='geen-vakantie'>Er is geen vakantie van $search beschikbaar</h1>";
        }
        ?>
    </div>
</div>
